...
$(function() {
  $('#reference').hover(function() {
    $('#popper').stop(true, false).fadeIn();
  }, function() {
    $('#popper').delay(1000).fadeOut();
  });
  ...
  function showmore() {
    swal({
      imageUrl: '//img.morejoy.cc/pc/img/xcx_qrcode.jpg',
      imageHeight: 200,
      imageWidth: 200,
      imageAlt: '小程序二维码',
      text: '打开微信扫一扫了解更多',
  ...
  function measure() {
    swal({
      html: '<div id="measure-submit-header"><h3>预约装修服务</h3>' +
      '<p>预约后我们将尽快与您联系(当前仅支持北京地区)</p></div>' +
      '<form id="measure-submit">'+
      '<div><input id="name" placeholder="请输入姓名" name="name" required type="text"></div>' +
      '<div><input id="mobile" placeholder="请输入手机号" name="mobile" required type="text"></div>' +
      '</form>',
      ...
      customClass: 'measure-box',
      showLoaderOnConfirm: true,
      preConfirm: () => {
        return new Promise((resolve) => {
          fetch('https://api.morejoy.cc/api/auth/anonymousreg', {
            method: 'post',
            body: new FormData(document.querySelector('#measure-submit'))
          })
            .then(response => response.json())
            .then(result => {
              if(result.code !== 200 && result.code !==292) {
